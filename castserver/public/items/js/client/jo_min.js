...
if(joGesture)
joGesture.load();var s=(typeof joScroller!=="undefined")?joScroller.prototype:null;var d=joDOM;if(s&&this.matchPlatform("tizen msie chrome safari bb10 firefox")){joDOM.addCSSClass(document.body,"nativescroll");s.onDown=function(){};s.setEvents=function(){};s.onUp=function(){};s.onMove=function(){};s.onClick=function(){};s.setPosition=s.setPositionNative;}
...
else{key[tag]=o;}}}},getPlatform:function(){return this.platform;},matchPlatform:function(test){return(test.indexOf(this.platform)>=0);},getVersion:function(){return this.version;},getLanguage:function(){return this.language;}};joDOM={enabled:false,get:function(id){if(typeof id==="string"){return joCache.exists(id)?joCache.get(id):document.getElementById(id);}
...
return t;},getBounds:function(node){var top=joDOM.pageOffsetTop(node);var left=joDOM.pageOffsetLeft(node);var bottom=top+node.offsetHeight;var right=left+node.offsetWidth;return{top:top,left:left,bottom:bottom,right:right,center:{x:left+(right-left)/2,y:top+(bottom-top)/2}};},setPosition:function(o,x,y,w,h){o.style.position="absolute";o.style.left=Math.floor(x)+"px";o.style.top=Math.floor(y)+"px";if(w&&h){o.style.width=Math.floor(w)+"px";o.style.height=Math.floor(h)+"px";}},attach:function(node,parent){parent.appendChild(node);},detach:function(node,parent){if(!parent)
...
return null;},exists:function(key){return this.cache[key];},clear:function(key){if(typeof this.cache[key]==='object')
...
this.container.removeChild(child);},isVisible:function(){return this.visible;},push:function(o){if(typeof o==="string")
o=joDOM.get(o);if(this.data&&this.data.length&&this.data[this.data.length-1]===o)
...
return;this.initValue(this.getValue());}});function joImage(url){var container=new Image();this.image=container;this.container=container;var self=this;this.loadEvent=new joSubject(this);this.errorEvent=new joSubject(this);function onload(){self.loadEvent.fire();}
function onerror(){self.errorEvent.fire();}
container.onload=onload;container.onerror=onerror;container.src=url;}
joImage.extend(joLabel,{tagName:"img",setImage:function(image){this.container.src="url("+image+")";}});function joCanvas(w,h){this.canvas=joDOM.create("canvas");if(this.canvas)
